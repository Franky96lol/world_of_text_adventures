...
global.bot.onText(/(\/start|\/login)/ , (data) => {
    ...
    if(!fs.existsSync(config.DB + '/accounts/' + user + '.json')){
    	    let playername = 'player' + fs.readdirSync(config.DB + '/accounts').length + '' + (Math.round(Math.random() *89) + 10);
    	    let acc = {
    	        id : user,
    	    	    heroname : playername,
    	    	    pos : {
    	    ...
    	    let message = 'Bienvenido a el campamento de entrenamiento de heroes ' + 
    	    ...
    	    const inline = { 
		          inline_keyboard: [ 
		              [{text : 'Continuar ⏩' , callback_data : 'ca_start'}]
	           ]
		     };
        ...
        return;
    	    ...
    	    if(global.bot.users[user] == undefined) global.bot.users[user] = (helper.readFile(config.DB + '/accounts/' + user + '.json'));
