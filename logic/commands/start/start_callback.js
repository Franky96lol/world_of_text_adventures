...
global.bot.on("callback_query", data => {
  ...
  switch (data.data) {
    ...
    case "ca_accept":
      global.bot.deleteMessage(user, data.message.message_id);
      global.bot.createAcc[user].skills.push({
        name: global.bot.createAcc[user].class,
        cd: 0
      });
      helper.writeFile(
        config.DB + "/accounts/" + user + ".json",
        global.bot.createAcc[user]
      );

      let hero = { owner: user };
      helper.writeFile(
        config.DB + "/heros/" + global.bot.createAcc[user].heroname + ".json",
        hero
      );
      ...
      delete global.bot.createAcc[user];
      if (!global.bot.users[user]) {
      } else {
        global.bot.sendMessage(
          user,
          "📜 El heroe `" +
            global.bot.users[user].heroname +
            "` a aparecido en el mundo."
        );
        global.bot.sendMessage(
          user,
          "💌 Para acceder al mundo utilice el comando /login"
        );
      }
      break;
  }
  ...
  if (global.bot.createAcc[user] != undefined) {
    if (global.bot.createAcc[user].insert_name) {
      ...
      if (
        ...
        global.bot.sendMessage(
          user,
          "⚠️ No siguio los pasos mencionados al escribir su nombre. \n\n📝 Inserte su nombre nuevamente :\n\n" +
        ...
        if (global.bot.createAcc[user].class != "?") {
          let _class = helper.readFile(
            config.DB + "/classes/" + global.bot.createAcc[user].class + ".json"
          );
          hclass = _class.name;
        }

        if (global.bot.createAcc[user].faction != "?") {
          let fact = helper.readFile(
            config.DB +
              "/factions/" +
              global.bot.createAcc[user].faction +
              ".json"
          );
          faction = fact.name;
        }
        ...
        let message =
          "📝 Esta es su ficha de heroe : \n\n" +
          "👤 Nombre : `" +
          global.bot.createAcc[user].heroname +
          "`\n\n" +
          "🔰 Facción : " +
          faction +
          "\n\n" +
...
function startMenu(user, chatid, messageid, old_message) {
  ...
  if (
    global.bot.createAcc[user].faction != "?" &&
    global.bot.createAcc[user].class != "?"
  ) {
    inline.inline_keyboard.push([
      { text: "Continuar ⏩", callback_data: "ca_continue" }
    ]);
  }
  let hclass = "???",
    faction = "???";
  ...
  if (global.bot.createAcc[user].faction != "?") {
    let fact = helper.readFile(
      config.DB + "/factions/" + global.bot.createAcc[user].faction + ".json"
    );
    faction = fact.name + "\n📜 " + fact.desc;
  }

  let message =
    "📝 Rellene su formulario para continuar : \n\n" +
    "👤 Nombre : `" +
    global.bot.createAcc[user].heroname +
    "`\n\n" +
    "🔰 Facción : " +
