...
class Map{
    ...
    addGFX(pos , _map , user){
    	    ...
    	    let gfxMap = '\t \t \t \t \t \t \t \t \t \t';
    	    let poa = this.getPlayersOnArea(pos , bot.users);
    	    let posx = 0 , posy = 0;
    ...
    getPlayersOnArea(pos , players){
        ...
        for(let pj in players){
            ...
            if((players[pj].pos.x > (pos.x -4)) && (players[pj].pos.x < (pos.x + 5)) && 
            	(players[pj].pos.y > (pos.y - 4)) && (players[pj].pos.y < (pos.y +5)) &&
            	(players[pj].pos._floor == pos._floor) && (players[pj].pos != pos)){
